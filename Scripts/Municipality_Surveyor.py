...
class MunicipalitySurveyor:
    ...
    async def fetch_tile(self, session, base_url, r_pos, c_pos, semaphore):
        async with semaphore:
            url = f"{base_url}/{c_pos}/{r_pos}{self.extension}"
            try:
    ...
    async def download_area(self, x_range, y_range):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            cir_tasks = [self.fetch_tile(session, self.cir_url, r, c, semaphore) for r in y_range for c in x_range]
            rgb_tasks = [self.fetch_tile(session, self.rgb_url, r, c, semaphore) for r in y_range for c in x_range]
    ...
    def run(self, limit=None, targets=None, include_height=True):
        ...
        for i, (idx, row) in enumerate(df_buurten.iterrows()):
            ...
            try:
                ...
                cir_data, rgb_data = loop.run_until_complete(self.download_area(x_range, y_range))
                loop.close()
