...
KEA_CLASSIFICATION_CONFIG = {
    ...
    "RMK_BKB_AHN4": {
        (31, 120, 180): (1, "Boomkroonbedekking Klasse 1", 1),
        (178, 223, 138): (2, "Boomkroonbedekking Klasse 2", 2),
        (51, 160, 44): (3, "Boomkroonbedekking Klasse 3", 3),
        (251, 154, 153): (4, "Boomkroonbedekking Klasse 4", 4),
        (227, 26, 28): (5, "Boomkroonbedekking Klasse 5", 5),
    },
}
...
class MunicipalitySurveyor:
    ...
    def fetch_buurten(self, targets=None):
        ...
        if targets:
            print(
                f"Fetching specific neighborhoods (2022 boundaries): {', '.join(targets)}..."
            )
        else:
            print(
                f"Fetching all buurten for {MUNICIPALITY_NAME} ({MUNICIPALITY_CODE}) - 2022 Boundaries..."
            )
        ...
        if targets:
            ...
            for t in targets:
                ...
                target_filters.append(
                    f"<ogc:PropertyIsEqualTo><ogc:PropertyName>{prop}</ogc:PropertyName><ogc:Literal>{t}</ogc:Literal></ogc:PropertyIsEqualTo>"
                )
        ...
        try:
            ...
            if df.empty:
                if targets:
                    print(
                        f"Warning: No matching neighborhoods found for the provided targets in {MUNICIPALITY_CODE}."
                    )
                else:
                    print(
                        f"Error: No land-based neighborhoods found for {MUNICIPALITY_CODE}."
                    )
                return df
    ...
    def initialize_wmts(self):
        ...
        try:
            ...
            self.brta_url = re.sub(
                r"/{TileCol}/{TileRow}\.(png|jpeg|jpg|png8)", "", url_brta
            )
            self.brta_extension = ".png" if ".png" in brta_template else ".jpg"
        except Exception as e:
            print(
                f" Warning: Could not initialize Topotijdreis 2022: {e}. Falling back to default background."
            )
        ...
        print(
            f"WMTS initialized. Resolution: {matrix.scaledenominator * 0.00028:.2f}m/px"
        )
    ...
    async def download_area(self, x_range, y_range, geometry):
        ...
        valid_indices = []
        for y_idx, r in enumerate(y_range):
            for x_idx, c in enumerate(x_range):
                tile_bounds = self.get_bbox_of_tile_indices([c], [r])
                tile_box = box(*tile_bounds)
                if tile_box.intersects(geometry):
                    valid_indices.append((y_idx, x_idx, r, c))

        semaphore = asyncio.Semaphore(5)
        async with aiohttp.ClientSession() as session:
            print(
                f"  Downloading {len(valid_indices)}/{len(x_range)*len(y_range)} tiles for area ({width}x{height} pixels)..."
            )
            cir_tasks = [
                self.fetch_tile(session, self.cir_url, r, c, semaphore, self.extension)
                for _, _, r, c in valid_indices
            ]
            rgb_tasks = [
                self.fetch_tile(session, self.rgb_url, r, c, semaphore, self.extension)
                for _, _, r, c in valid_indices
            ]
            brt_tasks = [
                self.fetch_tile(
                    session, self.brt_url, r, c, semaphore, self.brt_extension
                )
                for _, _, r, c in valid_indices
            ]
            brta_tasks = [
                self.fetch_tile(
                    session, self.brta_url, r, c, semaphore, self.brta_extension
                )
                for _, _, r, c in valid_indices
            ]
            bgt_tasks = [
                self.fetch_tile(
                    session, self.bgt_url, r, c, semaphore, self.bgt_extension
                )
                for _, _, r, c in valid_indices
            ]
            ...
            for idx, (y_idx, x_idx, r, c) in enumerate(valid_indices):
                r_start, c_start = y_idx * self.tileheight, x_idx * self.tilewidth
                if cir_results[idx]:
                    cir_array[
                        r_start : r_start + self.tileheight,
                        c_start : c_start + self.tilewidth,
                    ] = np.array(Image.open(BytesIO(cir_results[idx])))
                if rgb_results[idx]:
                    rgb_array[
                        r_start : r_start + self.tileheight,
                        c_start : c_start + self.tilewidth,
                    ] = np.array(Image.open(BytesIO(rgb_results[idx])))
                if brt_results[idx]:
                    img = Image.open(BytesIO(brt_results[idx]))
                    brt_array[
                        r_start : r_start + self.tileheight,
                        c_start : c_start + self.tilewidth,
                    ] = self._safe_image_to_array(img)
                if brta_results[idx]:
    ...
    def download_wms_data(self, bbox, width, height, layer, geometry):
        ...
        max_chunk_size = 2048
        cols = math.ceil(width / max_chunk_size)
        rows = math.ceil(height / max_chunk_size)
        x_min, y_min, x_max, y_max = bbox
        print(f"  Downloading KEA layer '{layer}' in chunks...")
        for i in range(rows):
            r_start = i * (height // rows)
            r_end = height if i == rows - 1 else (i + 1) * (height // rows)
            chunk_h = r_end - r_start
            for j in range(cols):
                c_start = j * (width // cols)
                c_end = width if j == cols - 1 else (j + 1) * (width // cols)
                chunk_w = c_end - c_start
                chunk_bbox = (
                    x_min + (c_start / width) * (x_max - x_min),
                    y_max - (r_end / height) * (y_max - y_min),
                    x_min + (c_end / width) * (x_max - x_min),
                    y_max - (r_start / height) * (y_max - y_min),
                )

                chunk_box = box(*chunk_bbox)
                ...
                try:
                    ...
                    full_map[r_start:r_end, c_start:c_end] = img_arr
                except Exception as e:
    ...
    def _reclassify_kea(self, rgba_array, layer_id):
        ...
        closest_indices = np.zeros(len(pixels), dtype=np.uint8)
        chunk_size = 10000000
        for i in range(0, len(pixels), chunk_size):
            chunk_pixels = pixels[i : i + chunk_size]
            dists = cdist(chunk_pixels, colors, "sqeuclidean")
            closest_indices[i : i + chunk_size] = np.argmin(dists, axis=1)

        # Map indices back to our specific Category IDs
        mapped_ids = np.array(ids, dtype=np.uint8)[closest_indices]
    ...
    async def _fetch_bgt_features(
        self, session, collection, bbox_rd, semaphore, max_features=10000
    ):
                    ...
                    while retry_count < 3 and not success:
                        try:
                            async with session.get(
                                next_url, params=current_params, timeout=60
                            ) as resp:
                                if resp.status != 200:
                                    if resp.status >= 500:
                                        ...
                                        print(
                                            f"    Warning: BGT {collection} returned status {resp.status}"
                                        )
                                        break
                            ...
                            async with session.get(
                                ...
                                if resp.status != 200:
                                    if resp.status >= 500:
                                        ...
                                        print(
                                            f"    Warning: BRT {collection} returned status {resp.status}"
                                        )
                                        break
    ...
    async def download_categorical_bgt(
        self, bbox_rd, width, height, transform, bgt_max_features=10000
    ):
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [
                self._fetch_bgt_features(
                    session, coll, bbox_rd, semaphore, max_features=bgt_max_features
                )
        ...
        for (coll, prop_name), features in zip(collection_config, results):
            ...
            for feat in features:
                ...
                label = (
                    f"{coll.capitalize()}: {subtype}"
                    if subtype and subtype != "Bestaand"
                    else coll.capitalize()
                )
            ...
            for val, geoms in shapes_by_id.items():
                rasterio.features.rasterize(
                    [(g, val) for g in geoms], out=mask, transform=transform
                )
    ...
    async def download_categorical_brt(
        self, bbox_rd, width, height, transform, brt_max_features=10000
    ):
        """Fetch BRT Top10NL feature collections via OGC API and rasterize them."""
        print(
            f"  Generating BRT Top10NL categorical raster (Peildatum: {PEILDATUM})..."
        )
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [
                self._fetch_brt_features(
                    session, coll_id, bbox_rd, semaphore, max_features=brt_max_features
                )
        ...
        for (coll_id, prop_name, base_name, all_touched), features in zip(
            collection_config, results
        ):
            if not features:
                continue
            shapes_by_id = {}
    ...
    def _brt_color(label):
        ...
        l = label.lower()
        if "geen brt" in l or "niet geclassificeerd" in l:
            return (235, 232, 225, 255)
        if "gebouw" in l:
            return (220, 75, 50, 255)
        if "spoor" in l:
            return (80, 50, 20, 255)
        if "inrichting" in l:
            return (190, 175, 155, 255)
        if l.startswith("weg"):
            if "autosnelweg" in l:
                return (60, 60, 60, 255)
            if "hoofdweg" in l:
                return (100, 100, 100, 255)
            if "lokale weg" in l or "straat" in l:
                return (175, 175, 175, 255)
            return (155, 155, 155, 255)
        if "water" in l:
            return (70, 145, 215, 255)
        if "terrein" in l:
            if "bos" in l:
                return (30, 100, 30, 255)
            if "grasland" in l:
                return (110, 195, 75, 255)
            if "akker" in l:
                return (195, 180, 90, 255)
            return (150, 195, 115, 255)
        if "functioneel" in l:
            return (160, 195, 215, 255)
    ...
    def _write_qgis_style(self, tiff_path, label_mapping, colormap):
        ...
        if not label_mapping or not colormap:
            return
        ...
        for label, val in sorted_items:
            if val in colormap:
                ...
                qml_lines.append(
                    f'        <paletteEntry color="{hex_color}" label="{safe_label}" value="{val}" alpha="{a}"/>'
                )
        ...
        for i in range(max(label_mapping.values()) + 1):
            ...
            safe_label = html.escape(label)
            qml_lines.append(
                f'    <row index="{i}"><field name="Value">{i}</field><field name="Label">{safe_label}</field></row>'
            )

        qml_lines.extend(["  </attributeTable>", "</qgis>"])
    ...
    def run(
        ...
        targets=None,
        include_height=True,
        include_bgt_cat=True,
        include_brt_cat=True,
        bgt_max_features=10000,
        brt_max_features=10000,
    ):
        ...
        if targets:
            ...
            mask = df_buurten["buurtcode"].str.lower().isin(targets_lower) | df_buurten[
                name_col
            ].str.lower().isin(targets_lower)
        ...
        for i, (idx, row) in enumerate(df_buurten.iterrows()):
            ...
            try:
                ...
                async def fetch_all_data():
                    results_area = await self.download_area(x_range, y_range, geom)
                    h, w = results_area[0].shape[0], results_area[0].shape[1]
                    transform = rio.transform.from_bounds(*bbox_tiles, w, h)
                    bgt_cat_res, lbl_mapping = (
                        (
                            await self.download_categorical_bgt(
                                bbox_tiles,
                                w,
                                h,
                                transform,
                                bgt_max_features=bgt_max_features,
                            )
                        )
                        if include_bgt_cat
                        else (None, {})
                    )
                    brt_cat_res, brt_lbl_mapping = (
                        (
                            await self.download_categorical_brt(
                                bbox_tiles,
                                w,
                                h,
                                transform,
                                brt_max_features=brt_max_features,
                            )
                        )
                        if include_brt_cat
                        else (None, {})
                ...
                (
                    (cir_data, rgb_data, brt_data, brta_data, bgt_data),
                    bgt_cat,
                    label_mapping,
                    brt_cat,
                    brt_label_mapping,
                ...
                # Geometry Masking
                geom_mask = rasterio.features.rasterize(
                    [(geom, 1)],
                    out_shape=(h, w),
                    transform=transform,
                    fill=0,
                ...
                cir_data[geom_mask == 0] = 255
                rgb_data[geom_mask == 0] = 255
                brt_data[geom_mask == 0] = 255
                brta_data[geom_mask == 0] = 255
                bgt_data[geom_mask == 0] = 255
                if bgt_cat is not None:
                    bgt_cat[geom_mask == 0] = 0
                if brt_cat is not None:
                    brt_cat[geom_mask == 0] = 0
                ...
                # Write CIR image
                with rio.open(output_cir, "w", count=3, **common_meta) as dst:
                    for b in range(3):
                        dst.write(cir_data[:, :, b], b + 1)
                        
                # Write RGB image
                with rio.open(output_rgb, "w", count=3, **common_meta) as dst:
                    for b in range(3):
                        dst.write(rgb_data[:, :, b], b + 1)
                ...
                with rio.open(
                    self.outdir / f"{base_filename}_BRT.tif", "w", **map_meta
                ) as dst:
                    for b in range(3):
                        dst.write(brt_data[:, :, b], b + 1)
                    dst.update_tags(
                        DESCRIPTION="BRT Top10NL (2022) - RGB", PEILDATUM=PEILDATUM
                    )
                with rio.open(
                    self.outdir / f"{base_filename}_BRTA.tif", "w", **map_meta
                ) as dst:
                    for b in range(3):
                        dst.write(brta_data[:, :, b], b + 1)
                    dst.update_tags(
                        DESCRIPTION="BRT Topotijdreis 2022 Background - RGB",
                        SOURCE="Topotijdreis",
                    )
                with rio.open(
                    self.outdir / f"{base_filename}_BGT.tif", "w", **map_meta
                ) as dst:
                    for b in range(3):
                        dst.write(bgt_data[:, :, b], b + 1)
                ...
                for layer_id in KEA_DATASET_IDS:
                    kea_data = self.download_wms_data(bbox_tiles, w, h, layer_id, geom)
                    kea_data[
                        geom_mask == 0
                    ] = 0  # Transparent background outside polygon

                    # 1. Save Visual RGBA (for QGIS display)
                    output_kea_layer = (
                        self.outdir / f"{base_filename}_KEA_{layer_id}.tif"
                    )
                    try:
                        with rio.open(output_kea_layer, "w", **kea_meta) as dst:
                            ...
                            dst.update_tags(
                                DESCRIPTION=f"KEA Visual Overlay: {layer_id}"
                            )
                    except PermissionError:
                        print(
                            f"    WARNING: Permission denied for {output_kea_layer.name}. Is it open in QGIS? Skipping..."
                        )
                        continue

                    # 2. Reclassify to Labelled Categorical (for ML)
                    kea_cat, kea_lbl_map, kea_colormap = self._reclassify_kea(
                        kea_data, layer_id
                    )
                    if kea_cat is not None:
                        output_kea_cat = (
                            self.outdir / f"{base_filename}_KEA_CAT_{layer_id}.tif"
                        )
                        ...
                        try:
                            with rio.open(output_kea_cat, "w", **cat_meta) as dst:
                                ...
                                dst.update_tags(
                                    DESCRIPTION=f"KEA Categorical Labels: {layer_id}",
                                    CATEGORICAL="YES",
                                )

                            # Generate sidecars for QGIS metadata
                            self._write_pam_metadata(
                                str(output_kea_cat), kea_lbl_map, colormap=kea_colormap
                            )
                            self._write_qgis_style(
                                str(output_kea_cat), kea_lbl_map, kea_colormap
                            )
                            print(
                                f"    SUCCESS: Reclassified KEA {layer_id} into {len(kea_lbl_map)} classes."
                            )
                        except PermissionError:
                            print(
                                f"    WARNING: Permission denied for {output_kea_cat.name}. Is it open in QGIS? Skipping..."
                            )
                ...
                if bgt_cat is not None:
                    ...
                    with rio.open(output_bgt_cat, "w", **cat_meta) as dst:
                        ...
                        colormap = {
                            ...
                            for l, val in label_mapping.items()
                        }  # simplified color logic for brevity
                        dst.write_colormap(1, colormap)
                    self._write_pam_metadata(
                        str(output_bgt_cat), label_mapping, colormap=colormap
                    )
                ...
                if brt_cat is not None:
                    ...
                    with rio.open(output_brt_cat, "w", **cat_meta) as dst:
                        dst.write(brt_cat, 1)
                        brt_colormap = {
                            val: self._brt_color(l)
                            for l, val in brt_label_mapping.items()
                        }
                        dst.write_colormap(1, brt_colormap)
                    self._write_pam_metadata(
                        str(output_brt_cat), brt_label_mapping, colormap=brt_colormap
                    )
                    self._write_qgis_style(
                        str(output_brt_cat), brt_label_mapping, brt_colormap
                    )

                # Calculate NDVI
                nir = cir_data[:, :, 0].astype(np.float32)
                red_b = cir_data[:, :, 1].astype(np.float32)
                ...
                # Scale NDVI to 0-200 and mask outside region
                ndvi_arr = np.round((ndvi + 1) * 100).astype(np.uint8)
                ndvi_arr[geom_mask == 0] = 255  # Mask outside
                ...
                with rio.open(output_ndvi, "w", **ndvi_meta) as dst:
                ...
                if include_height:
                    ...
                    chm.mask = chm.mask | (geom_mask == 0)
                    h_meta = common_meta.copy()
                    h_meta.update({"dtype": np.float32, "nodata": -9999.0, "count": 1})
                    with rio.open(
                        self.outdir / f"{base_filename}_CHM.tif", "w", **h_meta
                    ) as dst:
                        dst.write(chm.filled(-9999.0), 1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Surveyor for Súdwest-Fryslân - Version 4 (2022 Historical)"
    )
