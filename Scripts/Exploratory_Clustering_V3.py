...
Added new features for the model. It has to be optimized for the model. 
"""
...
def process_neighborhood(indir, target_name, outdir):
    ...
    kea_gvl_path = next(indir_path.glob(f"*{target_name}*_KEA_gevoelstemperatuur_2022.tif"), None)
    brt_cat_path = next(indir_path.glob(f"*{target_name}*_BRT_CAT.tif"), None)
    bgt_cat_path = next(indir_path.glob(f"*{target_name}*_BGT_CAT.tif"), None)
    kea_sgrijs_path = next(indir_path.glob(f"*{target_name}*_KEA_RMK_SchaduwGrijs_AHN4.tif"), None)
    kea_sgroen_path = next(indir_path.glob(f"*{target_name}*_KEA_RMK_SchaduwGroen_AHN4.tif"), None)
    AHN_trees_path = next(indir_path.glob(f"*{target_name}*_KEA_RMK_BKB_AHN4.tif"), None)
    kea_cat_landbedekking_path = next(indir_path.glob(f"*{target_name}*_KEA_CAT_BGV_landbedekking_2024.tif"), None)
    kea_opennietopen_path = next(indir_path.glob(f"*{target_name}*_KEA_BGV_ONO_2024.tif"), None)

    if not all([ndvi_path, chm_path, cir_path, kea_gvl_path, brt_cat_path, bgt_cat_path, kea_sgrijs_path, kea_sgroen_path, AHN_trees_path, kea_cat_landbedekking_path,
                kea_opennietopen_path]):
    ...
    with rio.open(kea_gvl_path) as src:
        gevoelstemp = src.read(1, out_shape=(ndvi.shape[0], ndvi.shape[1]),
                                resampling=Resampling.bilinear
        )
        temp_nodata = src.nodata
        temp_mask = (gevoelstemp == temp_nodata)
    
    with rio.open(brt_cat_path) as src:
        brt_cat = src.read(1, out_shape=ndvi.shape, resampling=Resampling.nearest)
        brt_mask = (brt_cat == src.nodata)
    
    with rio.open(bgt_cat_path) as src:
    # Use Resampling.nearest for BGT because the values are categorical IDs
        bgt_cat = src.read(1, out_shape=ndvi.shape, resampling=Resampling.nearest)
        bgt_nodata = src.nodata
        bgt_mask = (bgt_cat == bgt_nodata)

    with rio.open(kea_sgrijs_path) as src:
        shad_grijs = src.read(1, out_shape=ndvi.shape, resampling=Resampling.bilinear)
        shad_grijs_mask = (shad_grijs == src.nodata)

    with rio.open(kea_sgroen_path) as src:
        shad_groen = src.read(1, out_shape=ndvi.shape, resampling=Resampling.bilinear)
        shad_groen_mask = (shad_groen == src.nodata)
    
    with rio.open(AHN_trees_path) as src:
        tree_heights = src.read(1, out_shape=ndvi.shape, resampling=Resampling.bilinear)
        tree_nodata = src.nodata
        tree_mask = (tree_heights == tree_nodata)
    
    with rio.open(kea_cat_landbedekking_path) as src:
        landbedekking = src.read(1, out_shape=ndvi.shape, resampling=Resampling.nearest)
        bvg_nodata = src.nodata or 255
        bvg_mask = (landbedekking == bvg_nodata)
    
    with rio.open(kea_opennietopen_path) as src:
        openbaar_data = src.read(1, out_shape=ndvi.shape, resampling=Resampling.nearest)
        open_nodata = src.nodata or 255
        ONO_mask = (openbaar_data == open_nodata)

    # Combined mask
    invalid_mask = ndvi_mask | chm_mask | cir_mask | temp_mask | brt_mask | bgt_mask | shad_grijs_mask | shad_groen_mask | tree_mask | bvg_mask | ONO_mask
    ...
    features = np.column_stack([
        ...
        #brt_cat[valid_indices]
        #bgt_cat[valid_indices]
        #shad_grijs[valid_indices]
        #shad_groen[valid_indices],
        #tree_heights[valid_indices]
        #landbedekking[valid_indices]
        #openbaar_data[valid_indices]
    ]).astype(np.float32)
    ...
    stats_df = pd.DataFrame(cluster_centers, columns=['Mean_NDVI', 
                                                      'Mean_CHM', 
                                                      'Mean_NIR', 
                                                      'Mean_Red', 
                                                      'Mean_gevoelstemp', 
